...
def get_spiders():
    ...
    settings.set('SPIDER_MODULES', ['zillow.spiders'])
    settings.set('NEWSPIDER_MODULE', 'zillow.spiders')
...
def execute_spider():
    if dataset_entry.get() == '' or chosen_feed not in ['json', 'csv']:
        messagebox.showerror('Error', 'All entries are required')
        return
    
    try:
        feed_uri = f'{folder_path}/{dataset_entry.get()}.{chosen_feed}'
        
        settings = project.get_project_settings()
        settings.set('FEEDS', {feed_uri: {"format": chosen_feed}})
        print(feed_uri, chosen_feed)
...
spider_label.grid(row=0, column=0, sticky=W, pady=15, padx=10)
...
spiders_dropdown.grid(row=0, column=1, sticky=W, pady=10, padx=10, columnspan=2)
